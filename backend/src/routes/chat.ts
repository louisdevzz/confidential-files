...
import type { FullGeneratedCase, ChatRequest } from '../types/index.js';
...
router.post('/', async (req, res) => {
  try {
    ...
    // Gemini không hỗ trợ system role, nên thêm system prompt vào user message đầu tiên
    const safeMessages = messages.map((m) => ({
      role: m.role,
      content: sanitize(m.content),
    }));

    // Thêm system prompt vào message đầu tiên
    const geminiMessages: import('../types/index.js').ChatMessage[] = [
      { role: 'user', content: `[SYSTEM PROMPT - BẮT BUỘC TUÂN THỦ]: ${systemPrompt}\n\n[HỎI]: Đã đến lúc đối chất.` },
